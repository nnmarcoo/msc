...
impl VUMetersPane {
    pub fn new() -> Self {
        Self { rms_left: 0.0, rms_right: 0.0 }
    }
}
...
impl PaneView for VUMetersPane {
    fn update(&mut self, player: &Player, _art: &mut ArtCache) {
        ...
        if player.is_playing() {
            let coeff_l = if target_l > self.rms_left { ATTACK } else { FALL };
            let coeff_r = if target_r > self.rms_right { ATTACK } else { FALL };
...
fn draw_horizontal_pair(
    ...
    frame.fill_text(Text { content: "L".into(), position: Point::new(pad, my + ch * 0.5 - 5.0),               color: label_color, size: sz, ..Text::default() });
    frame.fill_text(Text { content: "R".into(), position: Point::new(pad, my + ch + ch_gap + ch * 0.5 - 5.0), color: label_color, size: sz, ..Text::default() });
    ...
    for &(pos, label) in DB_MARKERS {
        ...
        frame.fill_text(Text { content: label.into(), position: Point::new(tx + offset, scale_y + 5.0), color: label_color, size: 9.0.into(), ..Text::default() });
    }
}
...
fn draw_h_channel(
    ...
    for i in 0..n {
        ...
        frame.fill_rectangle(Point::new(sx, y + 1.0), Size::new(seg_w, height - 2.0), color);
    }
    ...
    frame.stroke(&border, Stroke::default().with_color(outline).with_width(1.0));
}

fn draw_vertical_pair(
    frame: &mut Frame,
    theme: &Theme,
    bounds: Rectangle,
    rms_l: f32,
    ...
    let sz: iced::Pixels = 10.0.into();
    frame.fill_text(Text { content: "L".into(), position: Point::new(mx + cw * 0.5 - 3.0,               my + mh + 3.0), color: label_color, size: sz, ..Text::default() });
    frame.fill_text(Text { content: "R".into(), position: Point::new(mx + cw + ch_gap + cw * 0.5 - 3.0, my + mh + 3.0), color: label_color, size: sz, ..Text::default() });

    let scale_x = mx + mw + 4.0;
    for &(pos, label) in DB_MARKERS {
        let ty = my + mh - pos * mh;
        frame.fill_rectangle(Point::new(scale_x, ty), Size::new(4.0, 1.0), tick_color);
        frame.fill_text(Text { content: label.into(), position: Point::new(scale_x + 6.0, ty - 4.5), color: label_color, size: 9.0.into(), ..Text::default() });
    }
}
...
fn draw_v_channel(
    ...
    for i in 0..n {
        ...
        let color = bar_color(theme, seg_pos, seg_pos <= rms);
        frame.fill_rectangle(Point::new(x + 1.0, sy), Size::new(width - 2.0, seg_h), color);
    }
